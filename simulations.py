...
def gradient_cost_l2(F, D, H, V, learning_batch, alphaF, alphaD,lambdaE):
    ...
    return ((2 * (D@F + H@Vminus - Vplus) @ F.T*lambdaE #/ (Nd*Nt) # They multiply F.T by lambdaE
        + 2 * alphaD * D ).flatten())  #/ (Nd*Ne)


# set up the cost function: 
# c_L2 = (||DF + HV - V+||_2)^2 + alphaD*(||D||_2)^2 + alphaF*(||F||_2)^2
def cost_l2(F, D, H, V, learning_batch, alphaF, alphaD,lambdaE):
#     print("LAMBDA_E")
    '''
    F: 64 channels x time EMG signals
    V: 2 x time target velocity
    ...
    e = ( np.sum( (D@F + H@Vminus - Vplus)**2 )*lambdaE #/ (Nd*Nt) 
